...
obconfig () {
	...
	xmlstarlet ed -L -N a="$namespace" -u '/a:openbox_config/a:margins/a:left' -v 5 "$config"
	xmlstarlet ed -L -N a="$namespace" -u '/a:openbox_config/a:margins/a:right' -v 5 "$config"
}
...
change_dock() {
	cat > "$HOME"/.cache/plank.conf <<- _EOF_
		...
		dock-items=['xfce-settings-manager.dockitem', 'Alacritty.dockitem', 'thunar.dockitem', 'google-chrome.dockitem', 'visual-studio-code.dockitem']
		hide-delay=0
		hide-mode='auto'
		icon-size=32
...
# funct WALLPAPER
set_wallpaper 'nordic-fynx.jpg'

# funct STYLE FONT
change_polybar 'nordic-fynx' 'Iosevka Nerd Font:size=10;3' && "$polybar_path"/launch.sh

# funct STYLE FONT BORDER BORDER-RADIUS ICON (Change colors in funct)
change_rofi 'nordic-fynx' 'Iosevka 10' '0px' '0px' 'Numix-Apps'

# funct STYLE (network manager applet)
change_nm 'nordic-fynx'

# funct FONT SIZE (Change colors in funct)
change_terminal 'SauceCodePro Nerd Font Mono' '12'

# funct FONT (Change colors in funct)
# change_xfce_terminal 'Iosevka Custom 9'

# funct SCHEME FONT
change_geany 'nordic-fynx' 'Iosevka Custom 10'

# funct THEME ICON CURSOR FONT
change_appearance 'fynx-arc-material' 'fynx-arc-material' 'Fluent-dark' 'Noto Sans 10'

# funct THEME LAYOUT FONT SIZE (Change margin in funct)
obconfig 'fynx-arc-material' 'LIMC' 'SauceCodePro Nerd Font Mono' '10' 'menu-icons.xml' && openbox --reconfigure

# funct GEOMETRY FONT BORDER (Change colors in funct)
change_dunst '280x50-10+50' 'Iosevka Custom 10' '0'
